...
```{r wzorce braków danych, echo=FALSE}
...
1. Profil demograficzny nabywcy. Analiza wykazała, że wiek jest jednym z najsilniejszych determinantów zakupu. Najwyższy współczynnik konwersji obserwuje się w grupie wiekowej 26–45 lat, co potwierdził test Chi-kwadrat wskazujący na istotną statystycznie zależność (p < 0.001). Klienci w wieku emerytalnym (56+) oraz bardzo młodzi (poniżej 25 lat) wykazują znacznie mniejszą skłonność do zakupu. Dodatkowo, poziom wykształcenia (szczególnie stopień licencjata/bachelors) jest silnie skorelowany z decyzją o zakupie, co sugeruje, że główną grupą docelową są wykwalifikowani specjaliści.

2. Status materialny i stabilizacja życiowa. Decyzja o zakupie roweru jest ściśle powiązana ze stabilizacją majątkową.

Dochód: Choć rozkład dochodów nie jest normalny (potwierdzone testem Shapiro-Wilka), analiza testem nieparametrycznym wskazuje, że wyższe zarobki sprzyjają zakupom, choć nie są jedynym czynnikiem.

Nieruchomości: Istnieje wyraźna przewaga kupujących wśród właścicieli domów (Home Owner: Yes) w porównaniu do osób wynajmujących.

Samochody: Zaobserwowano odwrotną zależność w przypadku liczby posiadanych aut – osoby posiadające 2 lub więcej samochodów rzadziej decydują się na zakup roweru, co może wynikać z preferencji transportowych.

3. Zróżnicowanie regionalne. Region Ameryki Północnej dominuje zarówno pod względem liczby klientów, jak i wolumenu sprzedaży. Analiza macierzy Cramér’s V potwierdziła, że region jest jedną ze zmiennych najsilniej różnicujących zachowania klientów. Rynek europejski i pacyficzny wykazują odmienną specyfikę, co sugeruje konieczność stosowania zróżnicowanych strategii dla każdego obszaru geograficznego.

4. Jakość danych i metodologia. Zbiór danych wymagał wstępnego czyszczenia i imputacji (ok. 24% braków w wybranych zmiennych). Zastosowane metody imputacji (dominanta dla zmiennych kategorialnych, mediana dla liczbowych) pozwoliły na zachowanie spójności rozkładów, co potwierdziła walidacja pakietem visdat. Wykryte braki normalności rozkładu dla zmiennych ilościowych (np. dochód) wymusiły zastosowanie odpowiednich testów nieparametrycznych, co zwiększyło rzetelność wnioskowania statystycznego.
