...
```{r tabela_shapiro, echo=FALSE, message=FALSE, warning=FALSE}
# Wymagane pakiety
library(broom)
library(dplyr)
library(knitr)
...
# 1. Wykonanie testu (zakładam, że dane2$income istnieje)
test_norm <- shapiro.test(dane2$income)

# 2. Porządkowanie wyników za pomocą broom::tidy()
tabela_norm <- tidy(test_norm) %>%
  mutate(
    # Formatowanie statystyki W
    statistic = round(statistic, 3),
    # Formatowanie p-value (np. <0.001)
    p.value_formatted = format.pval(p.value, digits = 3, eps = 0.001),
    # Dodanie interpretacji tekstowej w tabeli
    Interpretacja = if_else(p.value < 0.05, 
                            "Rozkład inny niż normalny", 
                            "Rozkład normalny")
  ) %>%
  select(method, statistic, p.value_formatted, Interpretacja)

# 3. Generowanie tabeli
kable(tabela_norm, 
      col.names = c("Test", "Statystyka W", "P-value", "Wniosek"),
      align = "lccc",
      caption = "Weryfikacja normalności rozkładu zmiennej dochód") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = F) %>%
  column_spec(3, bold = TRUE, color = ifelse(test_norm$p.value < 0.05, "red", "green")) %>%
  footnote(general = "P-value < 0.05 oznacza brak rozkładu normalnego.")
...
```{r macierz_pomylek_ulepszona, echo=FALSE, message=FALSE, warning=FALSE}
library(knitr)
library(kableExtra)
library(dplyr) 
...
prob <- predict(model, type = "response")
predicted_classes <- ifelse(prob > 0.5, "Kupi (Yes)", "Nie kupi (No)")

# Tworzymy tabelę
tabela_conf <- table(Prognoza = predicted_classes, Rzeczywistosc = df_clean$Purchased.Bike)

# Obliczenie dokładności do stopki
accuracy <- round(sum(diag(tabela_conf)) / sum(tabela_conf) * 100, 2)

kbl(tabela_conf, 
    col.names = c("Nie kupił", "Kupił"), 
    align = "cc",
    caption = "Macierz Pomyłek: Ocena trafności modelu") %>%
  
  # Stylizacja ogólna - full_width = T rozciągnie tabelę
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = TRUE, font_size = 16) %>%
  
  # Nagłówek górny (Rzeczywistość)
  add_header_above(c(" " = 1, "Rzeczywista decyzja klienta" = 2), 
                   bold = TRUE, background = "#f0f0f0") %>%
  
  # Dodanie etykiety dla kolumny z prognozami (Wiersze)
  pack_rows("Prognoza Modelu", 1, 2, label_row_css = "background-color: #666; color: white;") %>%
  
  # Formatowanie pierwszej kolumny (Nazwy wierszy)
  column_spec(1, bold = TRUE, width = "30%") %>%
  
  # KOLOROWANIE TRAFIEN
  # Zakładając układ: Wiersz 1=Nie, Wiersz 2=Tak
  # Komórka (1, 2) to True Negative (Dobrze przewidziane "Nie")
  column_spec(2, background = ifelse(rownames(tabela_conf) == "Nie kupi (No)", "#d1e7dd", "white")) %>%
  # Komórka (2, 3) to True Positive (Dobrze przewidziane "Tak")
  column_spec(3, background = ifelse(rownames(tabela_conf) == "Kupi (Yes)", "#d1e7dd", "white")) %>%
  
  # Stopka z wynikiem
  footnote(general = paste0("Całkowita dokładność modelu (Accuracy): ", accuracy, "%"),
           general_title = "Wniosek: ",
           footnote_as_chunk = T)
```
...
Model regresji logistycznej wskazał kluczowe czynniki wpływające na decyzję zakupu roweru. Najsilniejszymi predyktorami są: status cywilny (single częściej kupują), liczba posiadanych samochodów (im więcej aut, tym mniejsza skłonność do zakupu), liczba dzieci (wpływ negatywny) oraz dochód (wpływ pozytywny – rower jest dobrem normalnym).

Profil klienta o najwyższym potencjale zakupowym to osoba z wykształceniem wyższym, mieszkająca w Ameryce Północnej. Analiza wykazała natomiast brak istotnego wpływu płci, posiadania domu oraz zawodu na decyzję zakupową.

Jakość modelu jest umiarkowana (accuracy ok. 62,6%) – lepiej przewiduje brak zakupu niż zakup. Sugeruje to istnienie dodatkowych, nieujętych w danych czynników (np. styl życia, zainteresowania), które mogłyby poprawić trafność predykcji.
...
Projekt CycleInsight wykazał, że decyzja o zakupie roweru jest silnie uwarunkowana profilem socjodemograficznym i stylem życia klienta. Najważniejsze czynniki zwiększające prawdopodobieństwo zakupu to niższy dochód (szczególnie 20–40 tys. $), wykształcenie wyższe oraz zamieszkanie w Ameryce Północnej. Negatywnie wpływają natomiast liczba posiadanych samochodów i dzieci, co potwierdza rolę roweru jako alternatywnego środka transportu.

Analiza obaliła kilka intuicyjnych założeń: płeć, posiadanie domu i zawód nie mają istotnego znaczenia po uwzględnieniu innych zmiennych.

Idealny klient to singiel w wieku 36–45 lat, z wyższym wykształceniem, średnio-niskimi dochodami i ograniczonym dostępem do samochodu. Wnioski te sugerują, że strategia marketingowa powinna koncentrować się na korzyściach transportowych, jakości i prestiżu produktu oraz uwzględniać różnice regionalne, zamiast segmentacji według płci czy statusu mieszkaniowego.
